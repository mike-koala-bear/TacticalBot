...
class NNUEFeatureExtractor:
    ...
    def extract_features_bitboard(self, board, color: chess.Color) -> List[int]:
...
class SimpleNNUE:
    ...
    def evaluate_bitboard(self, board, color: chess.Color) -> int:
        """Evaluate position using bitboard representation"""
        features = self.feature_extractor.extract_features_bitboard(board, color)
    ...
    for epoch in range(epochs):
        ...
        for game in games:
            for i, board in enumerate(game):
                ...
                target = nnue._traditional_evaluate(board) / 100.0
